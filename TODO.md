...
- [x] Pod placement: `nodeSelector`/`tolerations`/`affinity`/`topologySpreadConstraints` passthrough at root, overridable per component and per queue/addon/job. Resolution is per-field and replace-only; a lower level cannot clear a root value. Hook jobs honour it too — otherwise a root toleration for a tainted pool deadlocks the ArgoCD sync at wave 20.
